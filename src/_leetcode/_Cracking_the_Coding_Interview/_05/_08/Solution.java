package _leetcode._Cracking_the_Coding_Interview._05._08;

import java.util.Arrays;

/**
 * @Description: 面试题 05.08. 绘制直线
 * 绘制直线。有个单色屏幕存储在一个一维数组中，使得32个连续像素可以存放在一个 int 里。
 * 屏幕宽度为w，且w可被32整除（即一个 int 不会分布在两行上），
 * 屏幕高度可由数组长度及屏幕宽度推算得出。请实现一个函数，绘制从点(x1, y)到点(x2, y)的水平线。
 * <p>
 * 给出数组的长度 length，宽度 w（以比特为单位）、直线开始位置 x1（比特为单位）、直线结束位置 x2（比特为单位）、直线所在行数 y。
 * 返回绘制过后的数组。
 * @Author: matreeix
 * @Date: 2020/7/31
 */

public class Solution {
    public static int[] drawLine(int length, int w, int x1, int x2, int y) {
        int start = w * y + x1;
        int end = w * y + x2;
        int[] res = new int[length];
        int idx1 = start / 32;
        int idx2 = end / 32;
        res[idx1] = -1 >>> (start % 32);
        res[idx2] = idx1 == idx2 ? res[idx1] >> (32 - end % 32 - 1) << (32 - end % 32 - 1)
                : -1 << (32 - end % 32 - 1);

        for (int i = idx1 + 1; i < idx2; i++)
            res[i] = -1;
        return res;
    }

    public static void main(String[] args) {
        System.out.println(Arrays.toString(drawLine(1, 32, 30, 31, 0)));
        System.out.println(Arrays.toString(drawLine(3, 96, 0, 95, 0)));
        System.out.println(Arrays.toString(drawLine(24, 96, 2, 19, 5)));
        System.out.println((-1 >>> 2) >> 12 << 12);

    }
}
